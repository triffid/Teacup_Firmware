...
int main (void)
{
	...
	for (;;)
	{
		...
		if (queue_full() == 0)
		{
			 ...
			 *
			 * Any data received over serial WILL be randomly distributed through the canned gcode, and you'll have a big mess!
			 *
			 * The solution is to either store gcode parser state with each source, or only parse a line at a time.
			 *
			 * This will take extra ram, and may be out of scope for the Teacup project.
			 *
			 * If ever print-from-SD card is implemented, these changes may become necessary.
			 */
			if (pgm_read_byte(canned_gcode[++canned_gcode_pos]) == 0)
				canned_gcode_pos = 0;
			else
				gcode_parse_char(pgm_read_byte(canned_gcode[canned_gcode_pos]));
			#endif /* CANNED_CYCLE */
		}
